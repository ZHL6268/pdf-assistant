...
当前阶段不直接迁移到目标栈，原因是本轮工作重点是先将前端结构整理到合理基线，避免功能实现和结构重构混在一起，增加返工成本。第一阶段已补齐共享配置、环境变量约定和 API 边界占位。第二阶段正在继续清理冗余代码，并把页面状态、共享路径配置和本地会话守卫接起来。当前展示层为了保持原始视觉效果，仍由 `src/App.tsx` 承载主页面布局；共享配置、会话和类型层已经先独立落地，后续再逐步拆分接入。
