...
当前实现范围仍以“前端结构整理 + 本地前端 MVP 闭环”为主，尚未接入真实认证、文件存储、PDF 解析、数据库和 OpenAI 服务。当前已完成第一阶段工程基线、第二阶段前端应用壳层整理和第三阶段文档管理前端实现：dashboard 上传入口、文档列表和当前文档选择已经在本地形成可测试闭环，同时保留原始前端 layout 作为展示基线。
