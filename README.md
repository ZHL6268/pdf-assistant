...
当前实现范围已经进入“真实后端接入”阶段。当前已完成第一阶段工程基线、第二阶段前端应用壳层整理、第三阶段本地文档管理前端实现、第四阶段真实认证与数据库基线、第五阶段真实文档上传与列表，并已进入第六阶段 PDF 文本提取与摘要生成：认证已切换到 Supabase Auth，dashboard 文档列表与上传链路已接入 Supabase Storage + `documents` 表，文档处理链路已接到 Supabase Edge Function。
