...
当前实现范围仍以“前端结构整理”为主，尚未接入真实认证、文件存储、PDF 解析、数据库和 OpenAI 服务。当前已完成第一阶段工程基线，并进入第二阶段前端应用壳层整理：删除死代码、收敛依赖、建立共享路径配置、基础页面导航能力，以及本地会话驱动的受保护页面边界，同时保留原始前端 layout 作为展示基线。
