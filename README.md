...
当前实现范围已经进入“真实后端接入”阶段。当前已完成第一阶段工程基线、第二阶段前端应用壳层整理、第三阶段本地文档管理前端实现、第四阶段真实认证与数据库基线、第五阶段真实文档上传与列表、第六阶段真实 PDF 摘要生成，并已进入第七阶段真实文档问答：认证已切换到 Supabase Auth，dashboard 文档列表与上传链路已接入 Supabase Storage + `documents` 表，文档处理链路已接到 Supabase Edge Function，并可直接基于 PDF 生成摘要和单文档回答。
...
5. 执行 [`supabase/migrations/0003_document_processing_error.sql`](./supabase/migrations/0003_document_processing_error.sql)
6. 执行 [`supabase/migrations/0004_message_indexes.sql`](./supabase/migrations/0004_message_indexes.sql)
7. 部署 `supabase/functions/process-document`
8. 部署 `supabase/functions/chat-document`
9. 在 Supabase secrets 中配置：
   - `OPENAI_API_KEY`
   - 可选 `OPENAI_MODEL`
